...
export class BillingService {
  ...
  async getUsageDashboard(userId: string): Promise<UsageDashboard> {
    ...
    const periodEnd = sub?.currentPeriodEnd ?? new Date(Date.now() + 24 * 60 * 60 * 1000);

    const [daily, periodTotal, chatPeek, summariesPeek, draftsPeek] = await Promise.all([
      this.usageRepo?.findDailyTotalsForPeriod(userId, periodStart, periodEnd) ?? [],
  ...
  async getPaymentHistory(userId: string, page: number, pageSize: number): Promise<PaymentHistoryPage> {
  ...
  async handleWebhookEvent(payload: RazorpayWebhookPayload, eventId: string): Promise<void> {
  ...
  private async processEvent(payload: RazorpayWebhookPayload): Promise<void> {
    ...
    switch (payload.event) {
      case RAZORPAY_WEBHOOK_EVENTS.SUBSCRIPTION_ACTIVATED: {
        if (!sub) break;
        await this.repo.updateSubscriptionStatus(sub.id, SubscriptionStatus.ACTIVE, {
      ...
      case RAZORPAY_WEBHOOK_EVENTS.SUBSCRIPTION_CANCELLED: {
        if (!sub) break;
        await this.repo.updateSubscriptionStatus(sub.id, SubscriptionStatus.CANCELLED);
        break;
      }

      case RAZORPAY_WEBHOOK_EVENTS.SUBSCRIPTION_HALTED: {
        if (!sub) break;
        await this.repo.updateSubscriptionStatus(sub.id, SubscriptionStatus.HALTED);
        break;
      }
    }
  }
